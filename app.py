...
{{
  ...
  "recommendation": "Balanced advice for the viewer."
    ...
    if verdict in caps:
        parsed["confidence"] = min(conf, caps[verdict])
...
{{
  ...
  "recommendation": "Balanced advice."
    ...
    try:
        ...
        v = challenged.get("verdict", "")
        if v in caps:
            challenged["confidence"] = min(challenged.get("confidence", 50), caps[v])
...
PREVIOUS VERDICT: {verdict} ({result.get('confidence')}%)
